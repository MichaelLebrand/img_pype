...
# Edited for use in the Starr lab.

# Added the following:
# call to SPM_image to view/edit nifti files
# Automated ACPC toolbox acpcdetect (origin in middle of AC and PC)
# DICOM to nifti conversion
# set_subjectGUI.py - GUI application to start a pipelined version of .prep_recon and .get_recon and all beforementioned
...
class freeCoG:
    ...
    def prep_recon(self, t1dicom='', ctdicom='', pipeline=0, origin = 0):
        ...
        if pipeline == 1:
            ...
            os.system('dcm2niix -z n -f rawT1 -o "%s" "%s"' % (self.acpc_dir, self.mriDicomDirectory))
            os.system('dcm2niix -z n -f rawCT -o "%s" "%s"' % (self.CT_dir, self.ctDicomDirectory))
    ...
    def setorigin(self):
        ...
        acpctxt = open(os.path.join(self.acpc_dir, 'T1_automated_ACPC.txt'))
        lines = acpctxt.readlines()

        # get locations from textfile
        for i in range(len(lines)):
            if '# AC (i,j,k) voxel location' in lines[i]:
                ac = lines[i + 1]
                pc = lines[i + 4]
        ...
        # getting values from textfile, remove spaces and converting to array
        ac = ac.rstrip('\n')
        ac = ac.split(' ')

        # remove redundant non-integer strings
        acStrRem = ac.count('')
        for i in range(acStrRem):
            ac.remove('')
        pc = pc.rstrip('\n')
        pc = pc.split(' ')
        ...
        print 'Voxel origin set to %s'%self.voxelorigin

        #set affine in nifti and change sform code to 2 (==alligned) and save file
        n1_img.header.set_sform(newAffine, code='scanner')
        n1_img._affine = newAffine
        print 'Saving automatically ACPC alligned T1 as T1.nii'
    ...
    def make_dural_surf(self, radius=3, num_iter=30, dilate=1.0):
        '''
    ...
    def mark_electrodes(self):
        ...
        of implantation.
        '''
        from .surface_warping_scripts import projectElectrodes as proj
        individual_elecs_dir = os.path.join(self.subj_dir,self.subj,'elecs','individual_elecs')
        if not os.path.isdir(individual_elecs_dir):
            print("Creating directory %s"%(individual_elecs_dir))
            os.mkdir(individual_elecs_dir)

        # creating initial dural surface
        dural_mesh = os.path.join(self.subj_dir, self.subj, 'Meshes', self.subj + '_' + self.hem + '_dural.mat')
        if not os.path.isfile(dural_mesh):
            num_iter = 30
            dilate = 0.5
        ...
        # this creates a smoothed version of the pial surface model and a file with the surface normals of this model
        SMEXduralCortexFile = os.path.join(self.mesh_dir, self.hem + '_sm_outer_dural_trivert.mat')
        if not os.path.isfile(SMEXduralCortexFile):
            surfaceNormalsFile = proj.createOuterSmoothed(self.surf_dir, self.hem, se_diameter=15)
            self.convert_fsmesh2mlab(mesh_name='sm_outer_dural')
        else:
            surfaceNormalsFile = os.path.join(self.surf_dir, self.hem + '.sm_outer_dural-normals')
            ans = raw_input(
                'Do you want to create a new instance of the smoothed dural surface used for projection? [y/n]')
            if ans.upper() == 'Y':
                proj.createOuterSmoothed(self.surf_dir, self.hem, se_diameter=15)
                self.convert_fsmesh2mlab(mesh_name='sm_outer_dural')

        print("Launching electrode picker")
        epicker = os.path.join(self.img_pipe_dir, 'electrode-registration-app','app', 'app.py')
        os.system('python %s %s %s'%(epicker, self.subj, self.hem))
    ...
    def convert_fsmesh2mlab(self, mesh_name='pial'):
        ...
        for h in hems:
            ...
            vert, tri, meta = nib.freesurfer.read_geometry(mesh_surf,read_metadata=True)
            # # convert vertices to RAS instead of Freesurfer Surface space
            # vert = vert + meta['cras']
    ...
    def reg_img(self, source='CT.nii', target='orig.mgz', fsl=0, smooth=0., reg_type='rigid',
                interp='pv', xtol=0.0001, ftol=0.0001):
        '''
        ...
        If the nipy coregistration doesn't work perfectly, specify the fsl parameters
        so the fsl package can be used to run the co-registration. Takes long (+5 min) but does a better job usually.
        (NB: make sure FSL is in set in your ~/.bashrc

        Parameters after fsl are arguments for nipy.algorithms.registration.histogram_registration.HistogramRegistration
        (for more information, sene help for nipy.algorithms.registration.optimizer).

        :param source: source image - the CT to be coregistered
        :param target: target image - the MRI where the CT gets coregsitered to
        :param fsl: use fsl coregistration or not. Default: 0, takes long but better coregistration. If set to 1 it
        coregisters it with 6 degrees of freedom. If fsl is set to 2 it coregister it with 12 degrees of freedom.
        :param smooth: (fsl == 0)        a smoothing parameter in mm
        :param reg_type: (fsl == 0) {'rigid', 'affine'}
            Registration type
        :param interp: (fsl == 0) {'pv','tri'}
            changes the interpolation method for resampling
        :param xtol: (fsl == 0) float
            tolerance parameter for function minimization
        :param ftol:  (fsl == 0) tolerance parameter for function minimization
        :return: none

        '''

        # set origin of CT image to middle
        if not os.path.isfile(os.path.join(self.CT_dir,'CT.nii')):
        ...
        outfilePackaged = outfile = os.path.join(self.CT_dir, 'r' + source)
        if os.path.isfile(outfile):
            try:
                os.remove(outfile)
            except:
                print "Couldn't remove %s, please remove manually." % outfile

        if os.path.isfile(outfilePackaged):
            try:
                os.remove(outfilePackaged)
            except:
                print "Couldn't remove %s, please remove manually." % outfilePackaged
        ...
        if fsl == 0:
            ...
            os.system('freeview -v "%s" "%s":colormap=heat:opacity=0.4'% (target_file, outfile))
        elif fsl == 1:
            os.system('ct2mri.sh "%s" "%s"'% (self.subj, source_file))
        elif fsl == 2:
            os.system('ct2mri12df.sh "%s" "%s"' % (self.subj, source_file))
        else:
            print "\n \n*************** \n" \
                  "INVALID FSL PARAMETER. Please specify a valid FSL parameter. \n" \
                  "*************** \n"

        if not os.path.isfile(outfile):
            if os.path.isfile(outfile+'.gz'):
                os.system('mri_convert %s %s'%(outfile+'.gz',outfile))
            else:
                print("Can't seem to find file: rCT.nii. Please check the subject's CT folder. ")
    ...
    def interp_grid(self, nrows=16, ncols=16, grid_basename='hd_grid'):
        ...
        Michael edit:
        I changed this to allow you to select the 2 corners of the grid when you are in approximately
        the same slice. If you have a grid that has 28 electrodes with 14 electrode rows, the first electrodes you 
        select will be #1 and #15 because they are located next to eachother. This saves you time down the line. 
        ...
        corner_nums = [0, nchans-nrows, 0+nrows-1, nchans-1]

        # Add the electrode coordinates for the corners
        for i in np.arange(4):
            elecmatrix[corner_nums[i],:] = corners[i,:]

        # Interpolate over one dimension (vertical columns from corner 1 to 3 and corner 2 to 4)
        # loop over x, y, and z coordinates
        for i in np.arange(3):
            elecmatrix[corner_nums[0]:corner_nums[2]+1,i] = np.linspace(elecmatrix[corner_nums[0],i], elecmatrix[corner_nums[2], i], nrows)
            elecmatrix[corner_nums[1]:corner_nums[3]+1,i] = np.linspace(elecmatrix[corner_nums[1],i], elecmatrix[corner_nums[3], i], nrows)
    ...
    # def strip_project_electrodes(self, elecfile_prefix = 'hd_grid', use_surface_normals = True, normDist = 25,
    #                        grid = True ,convex_hull = True, dilate = 0):
    #     '''
    ...
    #     use_surface_normal : bool, optional
    #     whether to use the mean normal vector of all vertices specified by normDist or whether to use mean normal vector
    #      (mean of the 4 normal vectors from the grids corner electrodes) as the projection direction.
    #     grid : bool, optional
    #     Whether the electrodes to project are from a grid that was interpolated using interp_grid()
    #     convex_hull : bool, optional
    #     Whether to project to the convex hull or the dural surface.  Often
    ...
    #
        # from .surface_warping_scripts import projectElectrodes as proj
    #
    #     print('Projection Params: \n\t Grid Name: %s.mat \n\t Use Surface Normal: %s \n\t'
    #                   ' Projecting To ConvexHull: %s \n\t'
    #           % (elecfile_prefix, use_surface_normals, convex_hull))
    #
    #     if grid:
    #         elecfile_name = elecfile_prefix + '_orig'
    #     else:
    #         elecfile_name = elecfile_prefix
    #         shutil.copy(os.path.join(self.elecs_dir, 'individual_elecs', elecfile_prefix + '.mat'),
    #                     os.path.join(self.elecs_dir, 'individual_elecs', elecfile_prefix + '_preProj.mat'))
    #
    #     # creating initial dural surface
    #     dural_mesh = os.path.join(self.subj_dir, self.subj, 'Meshes', self.subj + '_' + self.hem + '_dural.mat')
    #     if not os.path.isfile(dural_mesh):
    ...
    #
    #     # this creates a smoothed version of the pial surface model and a file with the surface normals of this model
    #     # sm_dural_mesh = os.path.join(self.subj_dir, self.subj, 'Meshes', self.subj + '_' + self.hem + '_sm_outer_dural.mat')
    #     SMEXduralCortexFile = os.path.join(self.mesh_dir, self.hem + '_sm_outer_dural_trivert.mat')
    #     if not os.path.isfile(SMEXduralCortexFile):
    #         surfaceNormalsFile = proj.createOuterSmoothed(self.surf_dir, self.hem, se_diameter=15)
    #         self.convert_fsmesh2mlab(mesh_name='sm_outer_dural')
    ...
    #             self.convert_fsmesh2mlab(mesh_name='sm_outer_dural')
    #
    #     # Load the variables and get the avg. surface normal per electrode from the smoothed, expanded (SMEX) dural
    #     # surface
    #     print('::: Loading Mesh data :::')
    #     SMEXduralCortex = scipy.io.loadmat(SMEXduralCortexFile)
    #     SMEXduralVert, SMEXduralTri = SMEXduralCortex['vert'], SMEXduralCortex['tri']
    #
    #     if grid:
    #         preProjElectrodes = scipy.io.loadmat(os.path.join(self.subj_dir, self.subj, 'elecs', 'individual_elecs', '%s_orig.mat'
    #                                                    %(elecfile_prefix)))['elecmatrix']
    #     else:
    #         preProjElectrodes = scipy.io.loadmat(os.path.join(self.subj_dir, self.subj, 'elecs', 'individual_elecs', '%s.mat' %(elecfile_prefix)))['elecmatrix']
    #
    #     direction = proj.calculateSurfaceNormals(SMEXduralVert, preProjElectrodes, surfaceNormalsFile, normDist)
    #
    #     if convex_hull:
    #         tri = scipy.spatial.Delaunay(SMEXduralVert).convex_hull
    #         projElectrodes = proj.projectElectrodes(tri, SMEXduralVert, preProjElectrodes, direction)
    #         scipy.io.savemat(
    #             os.path.join(self.subj_dir, self.subj, 'elecs', 'individual_elecs', '%s.mat' % (elecfile_prefix)),
    #             {'elecmatrix': projElectrodes})
    #     else:
    #         projElectrodes = proj.projectElectrodes(SMEXduralTri, SMEXduralVert, preProjElectrodes, direction)
    #         scipy.io.savemat(
    #             os.path.join(self.subj_dir, self.subj, 'elecs', 'individual_elecs', '%s.mat' % (elecfile_prefix)),
    #             {'elecmatrix': projElectrodes})
    #
    ...
    #                                  surf_type='dural', \
    #                                  num_iter=30, dilate=1.0, grid=True,
    #                                  convex_hull=True, newSurf = False):
    #     '''
    #     Project electrodes to the brain's surface to correct for deformation.
    #
    #     By default, projects the electrodes of a grid based on the mean normal vector of the four grid
    ...
    #         this can result in a smoother looking projection rather than a "wavy"
    #         looking grid.
    #     newSurf : bool, optional
    #         Whether to make a new surface to project the electrodes on. Could come in handy if the projections fall
    #         into the pial surface instead of on top of it.
    #
    #     Returns
    #     ---------
    #     None
    ...
    #
    #     dural_mesh = os.path.join(self.subj_dir, self.subj, 'Meshes', self.subj + '_' + self.hem + '_dural.mat')
    #     if ((surf_type=='dural' or convex_hull == False ) and not os.path.isfile(dural_mesh)) or newSurf:
    #         print('Creating dural surface mesh, using %d smoothing iterations'%(num_iter))
    #         self.make_dural_surf(num_iter=num_iter, dilate=dilate)
    #
    ...
    #
    #     cortex_dura = scipy.io.loadmat(os.path.join(self.subj_dir, self.subj, 'Meshes', '%s_%s_trivert.mat' % (self.hem, 'dural')))
    #     tri_dura, vert_dura = cortex_dura['tri'], cortex_dura['vert']
    #
    #
    #     print('::: Projecting electrodes to mesh :::')
    #     if grid:
    #         elecmatrix = scipy.io.loadmat(os.path.join(self.subj_dir, self.subj, 'elecs', 'individual_elecs', '%s_orig.mat'
    #                                                    %(elecfile_prefix)))['elecmatrix']
    #     else:
    #         elecmatrix = scipy.io.loadmat(os.path.join(self.subj_dir, self.subj, 'elecs', 'individual_elecs', '%s.mat' %(elecfile_prefix)))['elecmatrix']
    #
    #     print(direction)
    #     if convex_hull:
    ...
    #         elecs_proj_dura = project_electrodes_anydirection(tri_dura, vert_dura, elecmatrix, direction)
    #         scipy.io.savemat(os.path.join(self.subj_dir, self.subj, 'elecs', 'individual_elecs', '%s_dural.mat' % (elecfile_prefix)),
    #             {'elecmatrix': elecs_proj_dura})
    #     else:
    #         tri = scipy.spatial.Delaunay(vert).convex_hull
    #         elecs_proj = project_electrodes_anydirection(tri, vert, elecmatrix, direction)
    #         elecs_proj_dura = project_electrodes_anydirection(tri_dura, vert_dura, elecs_proj, direction)
    #         scipy.io.savemat(
    #             os.path.join(self.subj_dir, self.subj, 'elecs', 'individual_elecs', '%s_convexHull.mat' % (elecfile_prefix)),
    #             {'elecmatrix': elecs_proj})
    #         scipy.io.savemat(os.path.join(self.subj_dir, self.subj, 'elecs', 'individual_elecs', '%s.mat'%(elecfile_prefix)),{'elecmatrix':elecs_proj_dura})
    #
    #     print('::: Done :::')
    ...
    def make_elecs_all(self, input_list=None, outfile=None):
        ...
        Usage
        -----
        >>> patient.make_elecs_all(input_list=[('AD','AmygDepth','depth','amygdala_depth.mat'),
        ('G','Grid','grid','hd_grid.mat'),('nan','nan','nan',5)], outfile='test_elecs_all_list')
        ...
        if input_list != None:
            ...
            while done == False:
                num_empty_rows = raw_input('Are you adding a row that will be NaN in the elecmatrix? If not, press enter. '
                                           'If so, enter the number of empty rows to add: \n')
            ...
            outfile = raw_input('What filename would you like to save out to? Press enter to set to default (elecs_all.mat.) : \n')
            if not len(outfile):
                outfile ='elecs_all'
    ...
    def label_elecs(self, elecfile_prefix='elecs_all', atlas_surf='desikan-killiany', atlas_depth='destrieux', elecs_all=True):
        ...
        if atlas_surf == 'desikan-killiany':
            ...
            surf_atlas_flag = '--annotation aparc.a2009s'
        else:
            surf_atlas_flag = ''
    ...
    def warp_all(self, elecfile_prefix='elecs_all', warp_depths=True, warp_surface=True, template='fsaverage'):
        ...
        if warp_depths and warp_surface:
            ...
            self.plot_recon_anatomy_compare_warped(elecfile_prefix=elecfile_prefix,template=template)
    ...
    def compute_surface_warp(self, elecmatrix, anatomy=None, labelpath='tmp/', basename='',
        ...
        # # save nearest_verts to check
        # scipy.io.savemat('/home/michael/Documents/Subjects/test_tkd/nearest_verts.mat', {'elecmatrix': elecmatrix})
        ...
        for c in np.arange(len(surface_indices)):
            ...
            os.system('mri_label2label --srclabel ' + labelname + ' --srcsubject ' + self.subj + \
                      ...
                      ' --trgsurf pial --paint 2 pial --sd ' + self.subj_dir)
    ...
    def get_elecs(self, elecfile_prefix='elecs_all', roi=None):
    ...
    #                            'parstriangularis', 'superiorfrontal', 'rostralanteriorcingulate',
    #                            'caudalanteriorcingulate', 'frontalpole', 'insula'], hem=None, showfig=False)
    #     pial = self.roi('lh_pial',color=(0.8, 0.8, 0.8), opacity=0.8,representation='wireframe',gaussian=False)
    ...
    def plot_recon_anatomy(self, elecfile_prefix='elecs_all', template=None, showfig=True, screenshot=False,
        ...
        try:
            mlab.scalarbar = (elec_colors,'Brain Regions', elec_colors)
        except:
            print 'I tried'
    ...
    def plot_recon_anatomy_compare_warped(self, template='cvs_avg35_inMNI152', elecfile_prefix='elecs_all',
    ...
    def plot_all_surface_rois(self, plot_electrodes = True, bgcolor=(0, 0, 0), elecfile_prefix='elecs_all', size=(1200, 900), color_dict=None, screenshot=False, showfig=True,
                              **kwargs):
        """
        ...
        Parameters
        ----------
        plot_electrodes: bool
            if you want to plot the electrodes over the surfaces (default is True). Set False if no electrodes present!!
        bgcolor : tuple
        ...
        if plot_electrodes:
            ...
            ctmr_brain_plot.el_add(e['elecmatrix'],numbers=elec_numbers, label_offset=label_offset)
